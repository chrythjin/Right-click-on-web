# Chrome Web Store 등록 정보 (v0.2.0 → 스토어 출시용)
...
| **버전 (Version)** | 0.2.0 (스토어 출시 전 `1.0.0` 권장 — manifest.json 수정 필요) |
...
<h3>권한 안내</h3>
<p>이 확장은 모든 웹페이지에서 차단 스크립트를 완화하기 위해 호스트 권한(&lt;all_urls&gt;)을 사용합니다. 원격 서버에 데이터를 전송하거나 사용자 브라우징 데이터·입력 내용을 수집·저장하지 않습니다. 유일하게 저장되는 값은 ON/OFF 설정이며 chrome.storage.local에만 로컬 저장됩니다.</p>
```
...
>
> **데이터 처리**: 원격 서버 전송 없음, 사용자 브라우징 데이터 수집 없음, 설정만 로컬 `chrome.storage.local`에 저장.
